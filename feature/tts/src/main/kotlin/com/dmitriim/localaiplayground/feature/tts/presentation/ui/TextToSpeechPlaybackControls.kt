...
internal fun TextToSpeechPlaybackControls(
    state: TextToSpeechUiState, onPause: () -> Unit, onResume: () -> Unit, onStop: () -> Unit,
    onReplay: () -> Unit, onExport: () -> Unit, onShare: () -> Unit,
) {
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
            when {
                ...
                state.playback.status == SpeechPlaybackStatus.PLAYING -> { Button(onClick = onPause) { Text(stringResource(CoreUiR.string.tts_text_to_speech_controls_168)) }; OutlinedButton(onClick = onStop) { Text(stringResource(CoreUiR.string.tts_text_to_speech_controls_169)) } }
                state.playback.status == SpeechPlaybackStatus.PAUSED -> { Button(onClick = onResume) { Text(stringResource(CoreUiR.string.tts_text_to_speech_controls_170)) }; OutlinedButton(onClick = onStop) { Text(stringResource(CoreUiR.string.tts_text_to_speech_controls_171)) } }
        ...
        if (state.output != null) Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
