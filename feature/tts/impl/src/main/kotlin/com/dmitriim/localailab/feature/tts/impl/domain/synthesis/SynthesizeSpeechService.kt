...
@ContributesBinding(AppScope::class, binding = binding<SynthesizeSpeech>())
class SynthesizeSpeechService(
    ...
    override fun execute(request: SpeechSynthesisRequest): Flow<SpeechSynthesisEvent> = flow {
        Log.i(
            TAG,
        ...
        require(request.text.length <= SynthesizeSpeech.MAX_TEXT_CHARACTERS) {
            "Text is limited to ${SynthesizeSpeech.MAX_TEXT_CHARACTERS} characters for this playground."
        }
        cancelled.set(false)
        ...
        try {
            ...
            val fixedSpeaker = request.settings.voiceCondition as?
                com.dmitriim.localailab.ai.api.tts.TextToSpeechVoiceCondition.FixedSpeaker
    ...
    override suspend fun replay(audio: GeneratedAudioFile, volume: Float) {
    ...
    override fun unloadRuntime() = textToSpeechEngine.unload()
