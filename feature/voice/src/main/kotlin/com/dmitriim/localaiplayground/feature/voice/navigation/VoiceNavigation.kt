package com.dmitriim.localaiplayground.feature.voice.navigation
...
import com.dmitriim.localaiplayground.feature.voice.presentation.VoiceRoute
...
class VoiceNavigationEntryProvider : NavigationEntryProvider {
    override val target = NavigationTarget.VOICE_ASSISTANT
    override val hostDestination = TopLevelDestination.PLAYGROUND
    override val startKey: NavKey = VoiceKey

    override fun entryFor(key: NavKey, navigator: AppNavigator): NavEntry<NavKey>? =
        if (key == VoiceKey) NavEntry(key) { VoiceRoute() } else null
}
