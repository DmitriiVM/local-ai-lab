...
internal class ChatPromptPreparer(
    ...
    fun prepare(
        ...
        Log.i(
            TAG,
            "Chat prompt preparation started: turns=${turns.size}, " +
                "systemInstructions=${capabilities.systemInstructions}, " +
                "templateHandling=${capabilities.chatTemplateHandling}, exactTokenCounting=${tokenCounter != null}",
        )
        if (promptBudget == null) {
            return runtimeManagedPrompt(
                formatter = formatter,
                turns = included,
                config = config,
    ...
    private fun promptFormatter(
        capabilities: LlmEngineCapabilities,
    ): (List<LlmChatMessage>) -> String = when (capabilities.chatTemplateHandling) {
    ...
    private fun runtimeManagedPrompt(
        ...
        Log.i(
            TAG,
            "Chat prompt prepared with runtime-managed context: " +
                "promptTokens=$tokenCount, omittedMessages=0",
        )
        return PreparedChatPrompt(
            prompt = prompt,
            usage = ChatContextUsage(
...
private fun List<ChatTurn>.removableTurnCount(): Int {
    ...
    require(firstUser >= 0 && firstUser != indexOfLast { it.role == ChatTurnRole.USER }) {
        "The system prompt and latest user message do not fit with the requested output in " +
            "this context. Increase context size or reduce maximum output tokens."
    }
