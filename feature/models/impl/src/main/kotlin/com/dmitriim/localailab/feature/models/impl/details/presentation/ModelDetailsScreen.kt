...
private fun ModelDetailsContent(
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            item {
                DetailsSection(
                    ...
                    size?.let {
                        DetailValue(
                            "Size",
                            it.toDetailsReadableBytes()
                        )
                    }
                    DetailValue(
                        "Languages",
                        manifest.detailsLanguageSummary()
                    )
            ...
            catalogModel?.let { model -> item {
                DownloadStorageDetails(
                    model
                )
            } }
            item {
                DetailsSection(
                    ...
                    DetailValue(
                        "Engine",
                        manifest.engineId.value
                    )
                    DetailValue(
                        "Format",
                        manifest.format.displayLabel()
                    )
                    manifest.architecture?.let {
                        DetailValue(
                            "Architecture",
                            it
                        )
                    }
                    manifest.quantization?.let {
                        DetailValue(
                            "Quantization",
                            it
                        )
            ...
            item {
                InstallationDetails(
                    installedModel,
                    uiState.validationFeedback[manifest.modelId]
                )
            }
