...
internal fun ModelActionBar(
    installedModel: InstalledModel?, transfer: ModelTransferState?, validating: Boolean,
    onDownload: () -> Unit, onPauseTransfer: () -> Unit, onResumeOnWifi: () -> Unit,
    onResumeOnAnyNetwork: () -> Unit, onCancelTransfer: () -> Unit, onValidate: () -> Unit, onDelete: () -> Unit,
) {
        ...
        Column(
            ...
            if (installedModel != null) {
                Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                    OutlinedButton(
                        onClick = onValidate, enabled = !validating, modifier = Modifier.weight(1f),
                ...
                is ModelTransferState.Failed -> { Text(transfer.message, color = androidx.compose.material3.MaterialTheme.colorScheme.error); Button(onClick = onDownload, modifier = Modifier.fillMaxWidth()) { Text(stringResource(CoreUiR.string.models_model_details_screen_59)) } }
                ModelTransferState.Idle, null -> ModelDownloadButton(onClick = onDownload, modifier = Modifier.fillMaxWidth())
            }
        }
    }
    if (confirmAnyNetwork) AlertDialog(
        onDismissRequest = { confirmAnyNetwork = false }, title = { Text(stringResource(CoreUiR.string.models_model_details_screen_60)) }, text = { Text(stringResource(CoreUiR.string.models_model_details_screen_61)) },
        confirmButton = { Button(onClick = { confirmAnyNetwork = false; onResumeOnAnyNetwork() }) { Text(stringResource(CoreUiR.string.models_model_details_screen_62)) } },
    ...
    if (confirmCancel) AlertDialog(
        onDismissRequest = { confirmCancel = false }, title = { Text(stringResource(CoreUiR.string.models_model_details_screen_64)) }, text = { Text(stringResource(CoreUiR.string.models_model_details_screen_65)) },
        confirmButton = { Button(onClick = { confirmCancel = false; onCancelTransfer() }) { Text(stringResource(CoreUiR.string.models_model_details_screen_66)) } },
